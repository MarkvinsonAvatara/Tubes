...
$check_user_sql = "SELECT id_pendaftar From dt_team WHERE id_pendaftar = " . $_SESSION['id'] . "";
$check_user = $con->query($check_user_sql);

//gete id_pendaftar from query 
$pendaftar = $check_user->fetch_assoc();

if ($pendaftar['id_pendaftar'] == $_SESSION['id']) {
    echo "
            <script type='text/javascript'>
            alert('Tim Anda sudah terdaftar di turnamen ini')
            window.location = '../views/register.php'
            </script>";
} else if (mysqli_num_rows($empty_matches) == 0) {
    echo "
    <script type='text/javascript'>
    alert('Maaf Tim Anda tidak dapat terdaftar di turnamen ini karena sudah penuh')
    window.location = '../views/register.php'
    </script>";
} else {
    ...
    if ($players_query) {
        ...
        echo "
            <script type='text/javascript'>
            alert('Tim Anda berhasil terdaftar di turnamen ini')
            window.location = '../views/match.php'
            </script>";
